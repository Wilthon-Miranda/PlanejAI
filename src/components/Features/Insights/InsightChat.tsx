...
export function InsightChat({
  ...
  const hasConversation =
    insight.conversation && insight.conversation.length > 0
  ...
  }, [insight.conversation?.length, isSending])
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      <div ref={scrollRef} className="max-h-[440px] overflow-y-auto pr-1">
          ...
          {hasConversation && (
            <div className="space-y-4">
              {insight.conversation?.map((message, index) => {
                ...
                const isLastMessage = index === insight.conversation!.length - 1
                const isLoadingLastResponse =
      ...
      >
        <textarea
          ...
          className="text-foreground placeholder:text-muted-foreground h-12 flex-1 resize-none bg-transparent px-3 py-2 text-sm outline-none"
        ...
        <button
          ...
          className="bg-primary text-primary-foreground flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
